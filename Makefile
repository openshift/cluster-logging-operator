...
export LOGGING_VERSION?=6.5
export VERSION=$(LOGGING_VERSION).0
export NAMESPACE?=openshift-logging
export LOKI_OPERATOR_CHANNEL?=stable-6.4

IMAGE_LOGGING_VECTOR?=quay.io/vparfono/vector:v0.54.0-rh
...
run:
	...
	@mkdir -p $(CURDIR)/tmp
	LOG_LEVEL=$(LOG_LEVEL) \
	RELATED_IMAGE_VECTOR=quay.io/vparfono/vector:v0.54.0-rh \
...
test-functional-benchmarker-vector: bin/functional-benchmarker
	...
	@out=$$(RELATED_IMAGE_VECTOR=quay.io/vparfono/vector:v0.54.0-rh bin/functional-benchmarker --image=quay.io/vparfono/vector:v0.54.0-rh --artifact-dir=/tmp/benchmark-test-vector 2>&1); if [ "$$?" != "0" ] ; then echo "$$out"; exit 1; fi
