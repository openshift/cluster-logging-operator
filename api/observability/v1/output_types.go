...
type CloudwatchAWSAccessKey struct {
	...
	// AssumeRole specifies an additional role to assume for sending logs.
	// This enables cross-account log forwarding by using the initial credentials to authenticate,
	// then assume a role in order to access cross-account services.
	//
	// +kubebuilder:validation:Optional
	// +nullable
