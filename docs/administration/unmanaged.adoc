...
[source,bash]
----
  oc edit configmap/clf-$NAME-config
----
[sidebar]
====
...
[source,bash]
----
  oc extract configmap/clf-$NAME-config --keys=vector.toml  --confirm
----
...
[source,bash]
----
  oc set data configmap/clf-$NAME-config --from-file=vector.toml=vector.toml
----
====
[start=3]
