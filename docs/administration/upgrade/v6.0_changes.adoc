...
namespace before removing the **ClusterLogging** resource named '**instance**' in the '**openshift-logging**' namespace
...
.Delete the CR before upgrading
[source]
----
...
After the operator upgrade is complete, recreate the CR.

.Recreate the CR after upgrading
[source,yaml]
----
