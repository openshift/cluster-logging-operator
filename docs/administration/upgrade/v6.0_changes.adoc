...
=== No Automated Update
Given the numerous combinations in which the logging solution can be configured, there is no automated upgrade provided by the *Cluster Logging Operator*. The existing v5.9 operator must be uninstalled before the new v6.0 operator can be installed.  (Alternatively, see the note below on changing the subscription channel)

Important::
If using the OperatorHub in the OCP console view, you can continue to collect and forward logs if you *DO NOT* check the box `Delete all operand instances for this operator` when uninstalling.  This will allow your existing collector pods to continue to run until you choose to remove them.

NOTE: Another option is to manually change the operator channel to `stable-6.0` under the `Subscription` tab in the console.  Doing this will trigger the olm process to remove v5.y and install v6.0.   Following this process, your existing resources will continue to run, but are no longer managed by the ClusterLogging CRs. These unmanaged resources can be removed once you are ready to create your new resources.

== Changes
The Cluster Logging Operator no longer provides a "one click" installation, in favor of administrators having more granular control over individual components.  Administrators must now explicitly deploy an operator for each component (log storage, visualization and collection)
...
=== Log Storage
LokiStack is the only managed log storage solution available for this release.  It is based upon the **loki-operator** and has been available in prior releases as the preferred alternative to the managed Elasticsearch offering.  The deployment
...
The *ClusterLogForwarder* now requires an administrator to provide a service account, with correct RBAC permissions.  This service account must be specified in the configuration.
...
If your previous collectors are deployed in the namespace *openshift-logging* then you've likely been using the service account *logcollector* created by earlier versions of the operator.
...
spec:
  ...
  - name: es-output-by-label # <2>
    ...
    elasticsearch:
      ...
      version: 6
      index: '{.openshift.label.my-log-index||"app"}-write' # <3>
    tls:
      ca:
        key: ca-bundle.crt
        secretName: collector
