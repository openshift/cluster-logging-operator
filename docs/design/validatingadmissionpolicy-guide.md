...
6. **Redeploy the right thing.** If you change the CEL manifests, roll the
   operator so it re-reconciles. If you change the Go reconcile logic,
   rebuild/redeploy the operator image. A stale operator with new manifests
   (or vice-versa) produces confusing results.
...
| e2e (live cluster) | `test/e2e/collection/admission/` | End-to-end with `oc create --as` impersonation against a real OpenShift cluster. |
