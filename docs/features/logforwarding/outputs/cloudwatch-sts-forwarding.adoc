          ...
          iamRole:
            ...
            assumeRole:
              ...
              externalID:
                secretName: cross-account-secret
                key: external_id
              # Note: sessionName is automatically generated
  pipelines:
...
==== AssumeRole Authentication Fields
...
Session names for assumed role sessions are automatically generated by the operator to provide meaningful identification in AWS CloudTrail logs for auditing purposes.

**Automatic Session Name Generation:**
The operator generates session names using cluster metadata with the format:
...
3. **Session Names**: Use descriptive session names for auditing in CloudTrail
