...
NOTE: When using the ViaQ data model, audit logs include `verb` as a default Loki stream label (in addition to the global defaults). This allows queries such as `{log_type="audit", verb="create"}` without parsing every log line as JSON. Users who fully replace audit `labelKeys` with `ignoreGlobal: true` must add `verb` themselves if they still want that stream label.
