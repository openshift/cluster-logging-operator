...
This is a configuration guide for the `ClusterLogForwarder` spec introduced to send logs to OTel receivers.


*OTLP* describes the *protocol* for encoding, transporting, and delivering `OTel` (open telemetry) data between sources using the
...
spec:
  ...
  outputs:
    - type: otlp  # <1>
      name: otel-collector
      otlp:
        url: 'http://my-otel-receiver-service:4318/v1/logs'  # <2>
        authentication:
          token:
            from: serviceAccount  # <3>
        tuning:
          compression: gzip  # <4>
      tls:
        insecureSkipVerify: true  # <5>
  pipelines:
    - name: my-pipeline
      inputRefs:
        - application
