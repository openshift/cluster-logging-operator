= NetworkPolicies for the Cluster Logging Operator and its Logging Components
...
In addition, you can use the `NetworkPolicy` defined in `config/network-policy/clo-permissive-network-policy.yaml` to configure the Cluster Logging Operator's network policy for restricted networks.

== Overview

When logging components are deployed, the operator can optionally create permissive `NetworkPolicy` resources. This **opt-in** feature ensures proper functionality even when:
...
When an `AdminNetworkPolicy` (ANP) is used in your cluster to enforce network restrictions, you may need to configure delegation to allow the operator and its components' `NetworkPolicy` resources to take precedence for their traffic.

=== Understanding the Hierarchy

The OpenShift network policy precedence (highest to lowest priority) is as follows:
...
==== Example: Delegating Cluster Logging Operator Traffic
```yaml
apiVersion: policy.networking.k8s.io/v1alpha1
kind: AdminNetworkPolicy
metadata:
  name: allow-clo-delegation
spec:
  priority: 50  # Lower number = higher priority. Adjust based on your cluster's ANP priority scheme
  subject:
    pods:
      # Target operator manager pod in openshift-logging namespace
      namespaceSelector:
        matchLabels:
          kubernetes.io/metadata.name: openshift-logging
      podSelector:
  ...
  ingress:
    - name: delegate-to-operator-ingress
      action: Pass  # Pass to Cluster Logging Operator NetworkPolicy
      from:
        - namespaces: {}
  egress:
    - name: delegate-to-operator-egress
      action: Pass  # Pass to Cluster Logging Operator NetworkPolicy
      to:
        - namespaces: {} 
        - networks:
