...
|outputs|array|  *(optional)* 	Outputs are named destinations for log messages. A `default` output is provided which forwards logs to the internal {product-title} log store. You can define outputs to forward to other stores or log processors, inside or outside the cluster.
|pipelines|array|  Pipelines forward the messages selected by a set of inputs to a set of outputs.
|serviceAccountName|string|  *(optional)* The `ServiceAccountName` field specifies the service account associated with the `ClusterLogForwarder` object.
|======================
...
|application|object|  *(optional)* 	Note: The following fields in this struct are deliberately _not_ `omitempty`. An empty field means enable that input type with no filter. Application, if present, enables named set of `application` logs that can specify a set of match criteria.
...
|name|string|  The name used to refer to the input of a pipeline.
