...
|maxRetryDuration|int|  *(optional)* MaxRetryDuration is the maximum time to wait between re-connect attempts after a connection failure.
|maxWrite|object|  *(optional)* MaxWrite limits the maximum payload in terms of bytes of a single &#34;send&#34; to the output.
|minRetryDuration|int|  *(optional)* MinRetryDuration is the minimum time to wait between attempts to re-connect after a failure.
|======================
