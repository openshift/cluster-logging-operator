...
|ignoreOlder|object|  IgnoreOlder specifies the maximum duration since the last modification
...
|azureMonitor|object|  DEPRECATED: Use AzureLogsIngestion instead. This output will be removed in a future release.
...
|timestampField|string|  TimestampField is the destination field (column) for the timestamp.
...
|tokenScope|string|  TokenScope is the token scope for dedicated Azure regions.
...
|type|string|  Type is the type of Azure authentication to configure.
...
|type|string|  Type is the type of authentication to configure for an AWS service.
...
|sourceType|string|  SourceType can be used to specify a pretrained or custom source type in Splunk, but can only be set when PayloadKey is defined.
