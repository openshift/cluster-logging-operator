...
svt_workdir=${WORKLOAD_DIR:-/tmp/svt}
mkdir -p $svt_workdir ||:
pushd $svt_workdir
...
for i in $(seq 300); do
  if [ "$(oc -n $NAMESPACE get pods --ignore-not-found --no-headers -l component=elasticsearch | grep '2/2.*Running' | wc -l)" == "$TOT_ESNODES" ] ; then
    running=true
    break
  else
    sleep 1.0
...
running=""
for i in $(seq 300); do
  if [ "$(oc -n $NAMESPACE get pods --no-headers -l component=fluentd jsonpath='{range .items[*]}{.status.phase}{\"\n\"}{end}' | wc -l)" == "$tot_nodes" ] ; then
    running=true
    break
  else
    sleep 1.0
...
echo "Starting SVT test using $MSG_PER_SEC msg/s generated by $TOT_APPS applications"
...
for (( i=1; i <= $TOT_ITERATIONS; i++ )) ; do
  ...
  if WORKLOAD_DIR=$svt_workdir/workloads \
     ...
     ORIGIN_TESTS_VERSION=4.5 \
     NUM_PROJECTS=$TOT_APPS \
     ansible-playbook -v -i $WORKLOAD_DIR/inventory $WORKLOAD_DIR/workloads/$PLAYBOOK \
     ; then
