...
os::test::junit::declare_suite_start "[LogFilesMetricsExporter] Suite"

start_seconds=$(date +%s)

TEST_DIR=${TEST_DIR:-'./test/e2e/logfilesmetricexporter/'}
...
for dir in $(ls -d $TEST_DIR); do
  ...
  if CLEANUP_CMD="$( cd $( dirname ${BASH_SOURCE[0]} ) >/dev/null 2>&1 && pwd )/../../test/e2e/logfilesmetricexporter/cleanup.sh $artifact_dir" \
    ...
    go test -count=1 -parallel=1 -timeout=5m "$dir" -ginkgo.noColor -ginkgo.trace -ginkgo.slowSpecThreshold=300.0 | tee -a "$artifact_dir/test.log" ; then
    os::log::info "======================================================="
