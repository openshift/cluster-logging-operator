...
// appendAssumeRoleKeys adds any assume role spec to the list of keys
func appendAssumeRoleKeys(auth *obsv1.CloudwatchAuthentication, keys []*obsv1.SecretReference) []*obsv1.SecretReference {
	...
	switch auth.Type {
	case obsv1.CloudwatchAuthTypeAccessKey:
		if auth.AWSAccessKey.AssumeRole != nil {
			keys = append(keys, &auth.AWSAccessKey.AssumeRole.RoleARN)
		}
	case obsv1.CloudwatchAuthTypeIAMRole:
		if auth.IAMRole.AssumeRole != nil {
			keys = append(keys, &auth.IAMRole.AssumeRole.RoleARN)
		}
	}
	return keys
}
