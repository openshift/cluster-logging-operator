...
func (f *Factory) ReconcileInputServices(er record.EventRecorder, k8sClient kubernetes.Client, k8sReader kubernetes.Reader, namespace string, owner metav1.OwnerReference, visitors func(o runtime.Object)) error {
	...
	for _, input := range f.ForwarderSpec.Inputs {
		...
		if input.Receiver != nil {
			...
			if err := network.ReconcileInputService(er, k8sClient, namespace, serviceName, f.ResourceNames.CommonName, serviceName, listenPort, listenPort, input.Receiver.Type, owner, visitors); err != nil {
				return err
			}
		}
	}
