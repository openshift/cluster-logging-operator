...
`,
		...
		Entry("with username,password, sasl_over_ssl, passphrase (ssl_client_cert_key_password in kafka plugin conf) and sasl.mechanisms (scram_mechanism in kafka plugin conf) ", helpers.ConfGenerateTest{
			CLFSpec: logging.ClusterLogForwarderSpec{
				Outputs: []logging.OutputSpec{
					{
						Type: logging.OutputTypeKafka,
