...
`
	parseReceiverStructuredAuditTimestamp = `
audit_time, audit_err = parse_timestamp(to_string(._internal.structured.stageTimestamp) ?? "", "%+")
if audit_err != null {
  audit_time, audit_err = parse_timestamp(to_string(._internal.structured.requestReceivedTimestamp) ?? "", "%+")
}
if audit_err == null { ._internal.timestamp = audit_time }
`
