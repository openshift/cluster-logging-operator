...
type InternalElasticsearch elasticsearch.Elasticsearch

func (e InternalElasticsearch) Name() string {
	return "internalElasticsearchTemplate"
}

func (e InternalElasticsearch) Template() string {
...
func BasicAuth(o logging.OutputSpec, secret *corev1.Secret) []Element {
	...
	if o.Secret != nil {
		hasBasicAuth := false
		conf = append(conf, elasticsearch.BasicAuthConf{
			Desc:        "Basic Auth Config",
			ComponentID: helpers.FormatComponentID(o.Name),
		})
		if security.HasUsernamePassword(secret) {
			hasBasicAuth = true
			up := elasticsearch.UserNamePass{
