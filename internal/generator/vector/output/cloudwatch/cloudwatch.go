...
func authConfig(outputName string, auth *obs.CloudwatchAuthentication, options Options, secrets observability.Secrets) Element {
	...
	case obs.CloudwatchAuthTypeAccessKey:
		...
		if roleSpec := auth.AWSAccessKey.AssumeRole; roleSpec != nil {
			authConfig.AssumeRole.Value = vectorhelpers.SecretFrom(&roleSpec.RoleARN)
			if roleSpec.ExternalID != "" {
				// Optional externalID
				authConfig.ExternalID.Value = roleSpec.ExternalID
			}
		}
