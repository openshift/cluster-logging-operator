...
if exists(kv.msg) {
  msg_str = ""
  if is_array(kv.msg) {
    msg_str = to_string(kv.msg[0])
  } else {
    msg_str = to_string!(kv.msg)
  }
  after_paren = split(msg_str, "(", limit: 2)
  if length(after_paren) > 1 {
    inner = replace!(after_paren[1], r').*', "")
    parts = split(inner, ":")
    if length(parts) > 1 {
      r.attributes = push(r.attributes, {"key": "log.sequence", "value": {"stringValue": to_string(parts[1])}})
    }
  }
}
`
