...
var _ = Describe("Generating vector config for Splunk output", func() {
	...
	DescribeTable("#New", func(expFile string, op utils.Options, visit func(spec *obs.OutputSpec)) {
		...
		id, sink, transforms := splunk.New(helpers.MakeID(outputSpec.Name), adapter, []string{"pipelineName"}, secrets, op)
		...
		Entry("with sourceType", "splunk_sink_sourcetype.toml", framework.NoOptions, true, func(spec *obs.OutputSpec) {
			spec.Splunk.SourceType = "my:custom:sourcetype"
		}))
})
