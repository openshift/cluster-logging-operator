...
func Remove(k8sClient client.Client, namespace, logstoreSecretName string) (err error) {
	...
	if err = RemoveCustomResource(k8sClient, namespace, constants.ElasticsearchName, logstoreSecretName); err != nil && !(errors.IsNotFound(err) || meta.IsNoMatchError(err)) {
...
func RemoveCustomResource(k8sClient client.Client, namespace, name, logstoreSecretName string) (err error) {
	cr := NewEmptyElasticsearchCR(namespace, name, logstoreSecretName)
