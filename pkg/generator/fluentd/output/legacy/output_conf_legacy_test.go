...
var _ = Describe("fluentd conf generation", func() {
	Describe("generate legacy fluentdforward conf", func() {
		...
		DescribeTable("", TestGenerateConfWith(f),
			Entry("Both legacy fluentd and legacy syslog", ConfGenerateTest{
...
`,
			...
			Entry("Mix of legacy and normal forwarding", ConfGenerateTest{
				CLFSpec: logging.ClusterLogForwarderSpec{
