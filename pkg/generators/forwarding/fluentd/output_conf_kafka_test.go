	...
	Context("for basic username/password auth", func() {
		It("should return a valid configuration", func() {
			...
			<match **>
			   ...
			   default_topic topic
			   use_event_time true
         username "#{File.exists?('/var/run/ocp-collector/secrets/my-secret/username') ? open('/var/run/ocp-collector/secrets/my-secret/username','r') do |f|f.read end : ''}"
         password "#{File.exists?('/var/run/ocp-collector/secrets/my-secret/password') ? open('/var/run/ocp-collector/secrets/my-secret/password','r') do |f|f.read end : ''}"
