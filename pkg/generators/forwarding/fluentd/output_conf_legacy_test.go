...
var _ = Describe("Generating fluentd legacy output store config blocks", func() {
	...
	Context("when manged logstore is defined", func(){
		BeforeEach(func() {
			var err error
			// This is copied straight from the defaults when nothing is defined
			// since there is a cyclic error trying to convert to something that
			// can be imported directly
			lfSpec = &logforward.ForwardingSpec{
		...
		It("should produce well formed fluent.conf writing to legacy and default stores", func() {
	...
	Context("when managed logstore is not defined", func() {
