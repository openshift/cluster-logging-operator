...
var _ = Describe("generating source", func() {
	...
	Context("for only logs.app source", func() {
		BeforeEach(func() {
			results, err = generator.generateSource(sets.NewString(logging.InputNameApplication), sets.NewString())
			Expect(err).To(BeNil())
	...
	Context("for only logs.infra source", func() {
		BeforeEach(func() {
			results, err = generator.generateSource(sets.NewString(logging.InputNameInfrastructure), sets.NewString())
			Expect(err).To(BeNil())
			Expect(len(results) == 1).To(BeTrue())
		})
	...
	Context("for only logs.audit source", func() {
		BeforeEach(func() {
			results, err = generator.generateSource(sets.NewString(logging.InputNameAudit), sets.NewString())
			Expect(err).To(BeNil())
	...
	Context("for all log sources", func() {
		...
		BeforeEach(func() {
			results, err = generator.generateSource(sets.NewString(logging.InputNameApplication, logging.InputNameInfrastructure, logging.InputNameAudit), sets.NewString())
			Expect(err).To(BeNil())
