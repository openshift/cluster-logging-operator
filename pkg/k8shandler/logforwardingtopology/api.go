...
const (
	...
	//LogForwardingEnhancedEdgeNormalizationTopology deploys multiple containers to each node to collect and normalize log messagges
	LogForwardingEnhancedEdgeNormalizationTopology = "enhancedEdgeNormalization"
...
type LogForwarderTopology interface {
	...
	Undeploy() error
	Name() string
	ProcessConfigMap(*v1.ConfigMap) *v1.ConfigMap
	ProcessPodSpec(*v1.PodSpec) *v1.PodSpec
	ProcessService(*v1.Service) *v1.Service
	ProcessServiceMonitor(*monitoringv1.ServiceMonitor) *monitoringv1.ServiceMonitor
}
