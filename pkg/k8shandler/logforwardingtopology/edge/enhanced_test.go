...
var _ = Describe("EdgeTopologyEnhanced", func() {
	BeforeEach(func() {
		os.Setenv("IMAGE_FLUENTBIT", "ATESTVALE")
	})
	var (
		topology = EdgeTopologyEnhanced{}
	)
	Context("#ProcessPodSpec", func() {

		It("Should add a collector container", func() {
			podSpec := &v1.PodSpec{}
			topology.ProcessPodSpec(podSpec)
	...
	Context("ProcessServiceMonitor", func() {
		It("should add monitoring of the collector container", func() {
			sm := &monitoringv1.ServiceMonitor{}
			topology.ProcessServiceMonitor(sm)
