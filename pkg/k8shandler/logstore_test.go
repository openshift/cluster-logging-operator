...
func TestSplitRolesWhenNodeCountIsGt3(t *testing.T) {
	...
	for _, val := range elasticsearchCR.Spec.Nodes {
		...
		if val.NodeCount == 4 {
			...
			if val.NodeCount == 3 {
				expectedNode := elasticsearch.ElasticsearchNode{
					Roles: []elasticsearch.ElasticsearchNodeRole{"client", "master"},
				}

				if !areNodeRolesSame(expectedNode, val) {
					t.Errorf("Exp. the roles to be %q but was %q", expectedNode.Roles, val.Roles)
				} else {
					clientMasterFound = true
				}
			} else {
				t.Errorf("Exp. the NodeCount to be %q or %q but was %q", 3, 4, val.NodeCount)
			}
		}
	}

	if !clientMasterFound {
		t.Errorf("Exp. client master node was not found")
	}
