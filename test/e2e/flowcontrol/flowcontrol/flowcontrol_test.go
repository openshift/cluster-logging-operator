...
var _ = Describe("[E2E] FlowControl", func() {
	...
	It("using flow control policies at input and output", func() {
		...
		clf.Spec.Pipelines = []loggingv1.PipelineSpec{
			{
				...
				Name:       "group-drop-policy-at-application",
			},
			{
				InputRefs:  []string{"custom-app-1"},
				OutputRefs: []string{"loki-0"},
				Name:       "container-drop-policy-at-application",
			},
			{
				InputRefs:  []string{loggingv1.InputNameApplication},
				OutputRefs: []string{"loki-1"},
				Name:       "drop-policy-at-loki",
			},
		}
		...
		promQuery = fmt.Sprintf(VectorCompSentEvents, "group-drop-policy-at-application")
		ExpectMetricsWithinRange(GetCollectorMetrics(promQuery), 0, 102)

		promQuery = fmt.Sprintf(SumMetric, VectorCompSentEvents)
		promQuery = fmt.Sprintf(promQuery, "container-drop-policy-at-application") // Max number of logs allowed per second is 10 * 100 lines/sec
	...
	It("using flow control policies at both input and output", func() {
		clf.Spec.Inputs = []loggingv1.InputSpec{
			{
				Name: "custom-app3",
				Application: &loggingv1.Application{
		...
		clf.Spec.Pipelines = []loggingv1.PipelineSpec{
			{
				InputRefs:  []string{"custom-app3"},
				OutputRefs: []string{"loki-1"},
				Name:       "group-drop-application-drop-loki",
			},
			{
				InputRefs:  []string{"custom-app3"},
				OutputRefs: []string{"loki-2", loggingv1.OutputNameDefault},
				Name:       "drop-application-ignore-loki",
			},
		}
		...
		promQuery = fmt.Sprintf(VectorCompSentEvents, "group-drop-application-drop-loki")
		ExpectMetricsWithinRange(GetCollectorMetrics(promQuery), 0, 202)

		promQuery = fmt.Sprintf(VectorCompSentEvents, "drop-application-ignore-loki")
		ExpectMetricsWithinRange(GetCollectorMetrics(promQuery), 0, 202)

		promQuery = fmt.Sprintf(VectorCompSentEvents, "loki-2")
		ExpectMetricsNotFound(GetCollectorMetrics(promQuery))

		promQuery = fmt.Sprintf(VectorCompSentEvents, loggingv1.OutputNameDefault)
		ExpectMetricsWithinRange(GetCollectorMetrics(promQuery), 0, 202)
