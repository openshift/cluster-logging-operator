...
var _ = Describe("[ClusterLogForwarder] Forward to Lokistack", func() {
	...
	Context("when forwarding to Lokistack when dataModel is Otel and logs include trace context", func() {
		const traceLogGenName = "trace-log-generator"

		DescribeTable("should send logs with expected trace labels",
			func(messageFormat string, expectedTraceID, expectedSpanID, expectedFlags string, verifyTraceContextLabels bool) {
				lokiStackOut.LokiStack.DataModel = obs.LokiStackDataModelOpenTelemetry
				forwarder.Spec.Outputs = append(forwarder.Spec.Outputs, *lokiStackOut)
			...
			Entry("should extract and send trace context from key=value message",
				`Processing request trace_id="%s" span_id="%s" trace_flags="%s" status=200`,
				"0af7651916cd43dd8448eb211c80319c", "b7ad6b7169203331", "1",
				true,
			),
			Entry("should extract and send trace context from JSON message",
				`{"trace_id":"%s", "span_id":"%s", "trace_flags":"%s", "status":200}`,
				"0af7651916cd43dd8448eb211c80319c", "b7ad6b7169203331", "1",
				true,
			),
			Entry("should send logs without trace labels when all values invalid in JSON",
				`{"trace_id":"%s", "span_id":"%s", "trace_flags":"%s", "status":200}`,
				"not-valid", "not-valid", "0A",
				false,
			),
			Entry("should send logs without trace labels when all values invalid in key=value",
				`Processing request trace_id="%s" span_id="%s" trace_flags="%s" status=200`,
				"not-valid", "not-valid", "0A",
				false,
			),
