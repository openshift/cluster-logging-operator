...
var _ = Describe("[ClusterLogForwarder] Forward to Lokistack", func() {
	...
	BeforeEach(func() {
		...
		// Deploy minio, loki operator, lokistack
		if err = e2e.DeployMinioInNamespace(deployNS); err != nil {
			Fail(err.Error())
		}
	...
	It("should send logs to lokistack's OTLP endpoint when dataModel == Otel", func() {
		...
		res, err := lokistackReceiver.GetApplicationLogsByKeyValue(serviceAccount.Name, "kubernetes_namespace_name", logGenNS, framework.DefaultWaitForLogsTimeout)
		Expect(err).To(BeNil())
