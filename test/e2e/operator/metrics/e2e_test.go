...
var _ = Describe("Manager", Ordered, func() {
	...
	AfterAll(func() {
		if os.Getenv("DO_CLEANUP") == "" {
			...
			By("cleaning up the token secret")
			_, _ = oc.Literal().From("oc delete secret sa-token-secret -n %s", namespace).Run()

			By("cleaning up the metrics-reader ClusterRole and ClusterRoleBinding")
			_, _ = oc.Literal().From("oc delete clusterrole %s", metricsRoleName).Run()
			_, _ = oc.Literal().From("oc delete clusterrolebinding %s", metricsRoleBindingName).Run()
		}
	})
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			...
			// Construct absolute path to the role file from the project root
			// test/e2e/operator/metrics -> ../../../../config/rbac/metrics_reader_role.yaml
			roleFilePath, err := filepath.Abs(filepath.Join("..", "..", "..", "..", "config", "rbac", "metrics_reader_role.yaml"))
			Expect(err).NotTo(HaveOccurred(), "Failed to construct role file path")
			...
			By("creating a secret with the service account token")
			secretName := "sa-token-secret"
			_, err = oc.Literal().From("oc create secret generic %s --from-literal=token=%s -n %s",
				secretName, token, namespace).Run()
			if err != nil && !strings.Contains(err.Error(), "already exists") {
			...
			overrides := fmt.Sprintf(`{
				"spec": {
					"containers": [{
						...
						"command": ["/bin/sh", "-c"],
						"args": [
							"for i in $(seq 1 30); do curl -v -H \"Authorization: Bearer $(cat /var/run/secrets/sa-token/token)\" --cacert /var/run/secrets/kubernetes.io/serviceaccount/service-ca.crt https://%s.%s.svc.cluster.local:8443/metrics && exit 0 || sleep 2; done; exit 1"
						],
						"volumeMounts": [{
							"name": "service-ca",
			...
			}`, curlImage, metricsServiceName, namespace, secretName, serviceAccountName)
			_, err = oc.Run().
				Name("curl-metrics").
				WithNamespace(namespace).
				Image(curlImage).
...
func serviceAccountToken() (string, error) {
	...
	verifyTokenCreation := func(g Gomega) {
		...
		// Use WithRawOutput() to get the actual token, not the sanitized version
		output, err := oc.Literal().From("oc create --raw /api/v1/namespaces/%s/serviceaccounts/%s/token -f %s",
			namespace, serviceAccountName, tokenRequestFile).Run(oc.WithRawOutput())
		g.Expect(err).NotTo(HaveOccurred())
