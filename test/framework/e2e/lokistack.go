...
func (tc *E2ETestFramework) DeployMinioInNamespace(namespace string) error {
	clolog.V(1).Info("creating pvc, service, deployment for minio", "namespace", namespace)
	...
	_, err := tc.KubeClient.CoreV1().PersistentVolumeClaims(namespace).Create(context.TODO(), pvc, metav1.CreateOptions{})
	if err != nil {
		return err
	}
	tc.AddCleanup(func() error {
		return tc.KubeClient.CoreV1().PersistentVolumeClaims(namespace).Delete(context.TODO(), minioName, metav1.DeleteOptions{})
	})

	// Create service
	service := runtime.NewService(namespace, minioName)
	...
	_, err = tc.KubeClient.CoreV1().Services(namespace).Create(context.TODO(), service, metav1.CreateOptions{})
	if err != nil {
		return err
	}
	tc.AddCleanup(func() error {
	...
	// Create deployment
	deployment := runtime.NewDeployment(namespace, minioName)
...
`},
	...
	_, err = tc.KubeClient.AppsV1().Deployments(namespace).Create(context.TODO(), deployment, metav1.CreateOptions{})
	if err != nil {
		return err
	}

	tc.AddCleanup(func() error {
		return tc.KubeClient.AppsV1().Deployments(namespace).Delete(context.TODO(), minioName, metav1.DeleteOptions{})
	})

	// Ensure minio deployment is ready
	return tc.WaitForDeployment(namespace, minioName, defaultRetryInterval, defaultTimeout)
}
...
`, LokistackName, namespace, minioName+"-secret")
	...
	data := map[string][]byte{
		"endpoint":          []byte(fmt.Sprintf("http://minio.%s.svc:9000", namespace)),
...
func (ls LokistackLogStore) GetApplicationLogs(saName string, timeToWait time.Duration) ([]lokitesthelper.StreamValues, error) {
	...
	result, err := ls.QueryUntil(query, "", string(obs.InputTypeApplication), saName, 3, defaultTimeout)
	return result, err
}
...
func (ls LokistackLogStore) HasInfrastructureLogs(saName string, timeToWait time.Duration) (bool, error) {
	...
	result, err := ls.QueryUntil(query, "", string(obs.InputTypeInfrastructure), saName, 3, defaultTimeout)
	return len(result) > 0, err
}
...
func (ls LokistackLogStore) HasAuditLogs(saName string, timeToWait time.Duration) (bool, error) {
	...
	result, err := ls.QueryUntil(query, "", string(obs.InputTypeAudit), saName, 3, defaultTimeout)
	return len(result) > 0, err
}
