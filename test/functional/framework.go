...
type receiverBuilder func(f *FluentdFunctionalFramework, b *runtime2.PodBuilder, output logging.OutputSpec) error
...
func (f *FluentdFunctionalFramework) AddOutputContainersVisitors() []runtime2.PodBuilderVisitor {
	visitors := []runtime2.PodBuilderVisitor{
		func(b *runtime2.PodBuilder) error {
...
func (f *FluentdFunctionalFramework) DeployWithVisitor(visitor runtime2.PodBuilderVisitor) (err error) {
	visitors := []runtime2.PodBuilderVisitor{
		visitor,
	}
...
//Deploy the objects needed to functional Test
func (f *FluentdFunctionalFramework) DeployWithVisitors(visitors []runtime2.PodBuilderVisitor) (err error) {
	...
	configmap := runtime2.NewConfigMap(f.Test.NS.Name, f.Name, map[string]string{})
...
`
	...
	runtime2.NewConfigMapBuilder(configmap).
	...
	certs := runtime2.NewConfigMap(f.Test.NS.Name, certsName, map[string]string{})
	runtime2.NewConfigMapBuilder(certs).
	...
	service := runtime2.NewService(f.Test.NS.Name, f.Name)
	...
	role := runtime2.NewRole(f.Test.NS.Name, f.Name,
		v1.PolicyRule{
	...
	rolebinding := runtime2.NewRoleBinding(f.Test.NS.Name, f.Name,
		v1.RoleRef{
	...
	f.Pod = runtime2.NewPod(f.Test.NS.Name, f.Name)
	b := runtime2.NewPodBuilder(f.Pod).
		WithLabels(f.labels).
...
func (f *FluentdFunctionalFramework) addOutputContainers(b *runtime2.PodBuilder, outputs []logging.OutputSpec) error {
