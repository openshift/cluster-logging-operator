...
var _ = Describe("[Functional][Normalization] container metadata fallback from file path (LOG-9584)", func() {
	...
	BeforeEach(func() {
		...
		framework.VisitConfig = func(conf string) string {
			...
			re := regexp.MustCompile(`(?ms)\[sources\.input_application_container\]\n.*?(\n\[(?:transforms|sources\.internal))`)
			return string(re.ReplaceAll([]byte(conf), []byte(fakeContainerSource+"$1")))
		}
