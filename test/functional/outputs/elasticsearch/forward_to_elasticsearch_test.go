...
var _ = Describe(fmt.Sprintf("[Functional][Outputs][ElasticSearch] %s Logforwarding to ElasticSearch", testfw.LogCollectionType), func() {
	...
	Context("with $ symbol in vector config", func() {
		It("should authenticate to elasticsearch properly", func() {
			userName := "elastic"
			...
			functional.NewClusterLogForwarderBuilder(framework.Forwarder).
				FromInput(logging.InputNameApplication).
				ToElasticSearchOutputWithSecret(secret)

			defer framework.Cleanup()

			esVisitor := func(b *runtime.PodBuilder) error {
				return functional.AddESOutputWithBasicSecurity(password, b, framework.Forwarder.Spec.Outputs[0])
			}

			Expect(framework.DeployWithVisitor(esVisitor)).To(BeNil())

			Expect(framework.WritesApplicationLogs(1)).To(BeNil())
			raw, err := framework.GetLogsFromElasticSearchWithBasicSecurity(logging.OutputTypeElasticsearch, logging.InputNameApplication, userName, password)
			Expect(err).To(BeNil(), "Expected no errors reading the logs")
			Expect(raw).To(Not(BeEmpty()))
