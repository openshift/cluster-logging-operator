...
var _ = Describe("[Functional][Outputs][OTLP] Functional tests", func() {
	...
	DescribeTable("should send logs with trace context when dataModel == Otel", func(message string, traceContextSpec *obs.OtlpTraceContextSpec, expectedTraceID, expectedSpanID string, expectedFlag uint32) {
		obstestruntime.NewClusterLogForwarderBuilder(framework.Forwarder).
			FromInput(obs.InputTypeApplication).
			ToOtlpOutput(func(output *obs.OutputSpec) {
				output.OTLP.OtlpTraceContext = traceContextSpec
			})
		...
		Entry("should extract trace context using default pattern with '=' separator",
		...
		Entry("should extract trace context with custom field names (traceId, spanId, traceFlags)",
			`Incoming request traceId=abcdef1234567890abcdef1234567890 spanId=fedcba9876543210 traceFlags=01`,
			&obs.OtlpTraceContextSpec{
				AdditionalTraceIdFieldNames:    []string{"traceId"},
				AdditionalSpanIdFieldNames:     []string{"spanId"},
				AdditionalTraceFlagsFieldNames: []string{"traceFlags"},
			},
			"abcdef1234567890abcdef1234567890", "fedcba9876543210", uint32(1),
		),
		Entry("should extract trace context from W3C traceparent header using custom pattern",
			`Incoming request traceparent=00-abcdef1234567890abcdef1234567890-fedcba9876543210-01`,
			&obs.OtlpTraceContextSpec{
				CustomPatterns: []string{`traceparent=00-(?<trace_id>[0-9a-fA-F]{32})-(?<span_id>[0-9a-fA-F]{16})-(?<trace_flags>[0-9a-fA-F]{2})`},
			},
			"abcdef1234567890abcdef1234567890", "fedcba9876543210", uint32(1),
		),
		Entry("should pad short trace_id and span_id with leading zeros",
			`trace_id=abc123 span_id=def456 trace_flags=01`,
			nil,
			"00000000000000000000000000abc123", "0000000000def456", uint32(1),
		),
