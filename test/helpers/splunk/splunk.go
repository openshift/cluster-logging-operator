...
func WaitOnSplunk(f *functional.CollectorFunctionalFramework) {
	...
	gomega.Eventually(func() string {
		...
		return output
	}, 90*time.Second, 3*time.Second).Should(gomega.ContainSubstring("HEC is healthy"))
	...
	gomega.Eventually(func() string {
		...
		return output
	}, 15*time.Second, 3*time.Second).Should(gomega.SatisfyAll(
		gomega.ContainSubstring("splunkd is running"),
		gomega.ContainSubstring("splunk helpers are running"),
	))
}
