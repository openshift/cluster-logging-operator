...
func NewLogGenerator(namespace, name string, msgCount int, delay time.Duration, message string) *corev1.Pod {
	return NewMultiContainerLogGenerator(namespace, name, msgCount, delay, message, 1)
}

func NewMultiContainerLogGenerator(namespace, name string, msgCount int, delay time.Duration, message string, containerCount int) *corev1.Pod {
	condition := "true"
	if msgCount > 0 {
